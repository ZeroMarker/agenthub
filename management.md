...
> 创建日期：2026-06-24
> 更新日期：2026-08-06
> 状态：设计文档（六大业务模块 + overview 概览模块均有基础实现；审计/备份横切能力首波已完成）
> 变更：v0.4 移除独立 management 模块——统一入口/生命周期/批量操作归 package，权限与密钥归 config，成本监控归 session，插件归 skill；v0.5 保留 overview（概览）为独立只读模块；审计、备份/恢复、监控作为横切能力（非模块），见文末「跨模块操作」

## 概述

本文档定义 AgentHub 的模块管理规范：六大业务模块（package, config, prompt, session, skill, memory）+ overview（概览，只读聚合）的完整生命周期管理，包括数据模型、CRUD 操作、状态监控、审计日志、导入导出和备份恢复。审计、备份/恢复、监控是**横切能力**，不构成独立模块。
...
| Overview | ✅ 首波完成 | 只读聚合各模块状态/成本/审计 → 仪表盘（`agenthub status`、GUI 概览视图） |
| 横切能力 | ✅ 首波完成 | 审计日志（JSONL）、备份/恢复 |
